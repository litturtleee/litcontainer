...
func ParseContainerConfig(image, containerName string, cmd, mountVolumes, envs []string) (*Config,
	error) {
	volume, err := parseMountVolume(mountVolumes)
	if err != nil {
		logger.Error("Failed to parse container serverconfig: %v", err)
		return nil, fmt.Errorf("failed to parse container serverconfig: %w", err)
	}
	...
	logger.Debug("container serverconfig is :%v", containerConfig)

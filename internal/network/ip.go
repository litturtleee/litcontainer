...
func (c *Controller) allocateIP(subnet *net.IPNet) (net.IP, error) {
	c.mu.Lock()
	defer c.mu.Unlock()

	subnetBitmap, err := c.loadBitmap(subnet.String())
	if err != nil {
...
func (c *Controller) releaseIP(subnet *net.IPNet, ip net.IP) error {
	c.mu.Lock()
	defer c.mu.Unlock()

	bitmap, err := c.loadBitmap(subnet.String())
	if err != nil {
		logger.Error("load subnet bitmap failed: %v", err)
		return err
	}
	...
	return c.saveBitmap(bitmap)
}

func (c *Controller) IsAllocatedIP(subnet *net.IPNet, ip net.IP) (bool, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// Stats 统计网络信息
func (c *Controller) Stats(subnet *net.IPNet) (used, available int, err error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Controller) loadBitmap(cidr string) (*SubnetBitmap, error) {
	...
	subnetStr, err := c.db.Get(AllocatedIPKeyTable, cidr)
	if err != nil {
...
func (c *Controller) saveBitmap(subnetBitmap *SubnetBitmap) error {
	...
	err = c.db.Put(AllocatedIPKeyTable, key, val)
	if err != nil {

...
func ListNetworks() error {
	...
	err := db.WithBoltDB(DefaultNetworkDBPath, func(dbClient *db.BoltDB) error {
		var getAllErr error
		datas, getAllErr = dbClient.GetAll(DefaultNetworkTable)
		return getAllErr
	})
	if err != nil {
...
func GetNetworkFromDB(name string) (*Network, error) {
	...
	err := db.WithBoltDB(DefaultNetworkDBPath, func(dbClient *db.BoltDB) error {
		var getErr error
		jsonStr, getErr = dbClient.Get(DefaultNetworkTable, name)
		return getErr
	})
	if err != nil {
		logger.Error("get network from db failed: %v", err)
...
func (n *Network) Save() error {
	...
	if err := db.WithBoltDB(DefaultNetworkDBPath, func(dbClient *db.BoltDB) error {
		return dbClient.Put(DefaultNetworkTable, n.Name, networkStr)
	}); err != nil {
...
func (n *Network) Delete() error {
	err := db.WithBoltDB(DefaultNetworkDBPath, func(dbClient *db.BoltDB) error {
		return dbClient.Delete(DefaultNetworkTable, n.Name)
	})
	if err != nil {
...
func Connect(name string, epCfg *ContainerEndpointConfig) (*net.IP, error) {
	nw, err := GetNetworkFromDB(name)
	if err != nil {
		logger.Error("get network from db failed: %v", err)
		return nil, err
...
// Disconnect
// 当前项目容器停止就应该做这些
func Disconnect(name string, epCfg *ContainerEndpointConfig) error {
	nw, err := GetNetworkFromDB(name)
	if err != nil {
		logger.Error("get network from db failed: %v", err)
		return err
